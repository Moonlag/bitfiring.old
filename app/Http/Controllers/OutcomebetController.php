...
class OutcomebetController extends Controller
{
	use DBTrait;
	use AuthTrait;
	use CallBackTrait;
    ...
    public function __construct()
    {
        ...
        $this->middleware(function ($request, $next) {
			...
			$this->insert_callback('Outcomebet', json_encode($request->all()).json_encode($data).$_SERVER['REQUEST_URI'], json_encode($request->header()));	
	...
	public function getBalance($input)
	{
		...
		if(!isset($user_status->status) || $user_status->status != 1) {
			return $this->show_error("user_not_active", "User is disabled", 403, "outcomebet", $input);
		}
		...
		return $this->show_response($response_body, $input, 200, "outcomebet");
	...
	public function rollbackTransaction($input)
	{
		...
		if(!isset($bet->id)) {
			return $this->show_response([], $input, 200, "outcomebet");
		}		
		...
		return $this->show_response(json_decode ("{}"), $input, 200, "outcomebet");
	...
	public function withdrawAndDeposit($input)
	{
		...
		if(!isset($user_status->status) || $user_status->status != 1) {
			return $this->show_error(5, "User is disabled", 200, "outcomebet", $input);
		}	
		...
		if($user_wealth->balance < ($input['params']['withdraw']*$denomination->denomination)) {
			return $this->show_error(1, "ErrNotEnoughMoneyCode", 200, "outcomebet", $input);		
		}				
		
		if($denomination->altercode != $input['params']['currency']) {
			return $this->show_error(2, "ErrIllegalCurrencyCode", 200, "outcomebet", $input);		
		}	
		...
		if(($input['params']['withdraw']*$denomination->denomination) < 0) {
			return $this->show_error(3, "ErrNegativeDepositCode", 200, "outcomebet", $input);		
		} else {
			
			$current_balance = $current_balance-($input['params']['withdraw']*$denomination->denomination);
			if(!$bet_id) {
				$bet_id = $this->add_bet($input, $user_wealth, $game, "outcomebet", $denomination);
			}
			$tx_id  = $this->add_transaction($input, $user_wealth, $game, "outcomebet", $bet_id, -1, "withdraw", $denomination);
		...
		if($input['params']['deposit'] < 0) {
			return $this->show_error(4, "ErrNegativeWithdrawalCode", 200, "outcomebet", $input);		
		} elseif($input['params']['deposit'] > 0) {
			
			$current_balance = $current_balance+($input['params']['deposit']*$denomination->denomination);
			$tx_id  = $this->add_transaction($input, $user_wealth, $game, "outcomebet", $bet_id, 1, "deposit", $denomination);
