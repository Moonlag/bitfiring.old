...
class BelatraController extends Controller
{
	use DBTrait;
	use AuthTrait;
	use CallBackTrait;
    ...
    public function __construct()
    {
        ...
        $this->middleware(function ($request, $next) {
            ...
            if (1==1 || $request->hasHeader('x-request-sign', false) && $this->checkRequestSignature($request)) {
				
				$data = json_decode(file_get_contents('php://input'), true);
				$this->insert_callback('Belatra', json_encode($request->all()).json_encode($data).$_SERVER['REQUEST_URI'], json_encode($request->header()));	
    ...
    public function play(Request $request)
    {
		...
		$token = $this->get_game_session("token", $input['session_id']);			
    ...
    public function rollback(Request $request)
    {
		...
		foreach($input['actions'] as $action) {
			...
			$bet 	  	 = $this->get_bet("tx_id", $action['original_action_id'], 1);
