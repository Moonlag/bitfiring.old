...
class Edit extends Screen
{
    ...
    public function query($id, Request $request): array
    {
        ...
        if($request->get('type') == 2){
            $model = \App\Models\FreespinBonusModel::find($id);
            $model->type_id = 4;
            $model->game = \App\Models\FreespinBonusGamesModel::query()->where('fb_id', '=', $id)->get()->pluck('game_id');
        }else{
            $model = \App\Models\Bonuses::find($id);
        }

        $this->name = $model->title;
        if($model->image){
            $model->image = Image::make($model->image . ".png");
        }
    ...
    public function first_deposit($data, $currency): JsonResponse
    {
        $data = (array)$data;
        ...
        if ($validator->passes()) {
            \App\Models\Bonuses::query()->where('id', $this->bonus_id)->update($query);
    ...
    public function n_deposit($data, $currency): JsonResponse
    {
        ...
        \App\Models\Bonuses::query()->where('id', $this->bonus_id)->update($query);
        IssuePlayerLimit::query()->where('bonus_id', $this->bonus_id)->delete();
    ...
    public function freespin($data, $currency, $id)
    {
        $data = (array)$data;
        ...
        \App\Models\FreespinBonusModel::query()->where('id', $id)->update($query);

        $query = [];

        foreach ($data[0]->game as $game) {
            $query[] = [
                'fb_id' => $id,
                'game_id' => $game
            ];
        ...
        \App\Models\FreespinBonusGamesModel::query()->where('fb_id', '=', $id)->delete();
        \App\Models\FreespinBonusGamesModel::query()->insert($query);
