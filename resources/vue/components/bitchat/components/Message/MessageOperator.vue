<template>
  ...
  <div class="chat-flex chat-items-center chat-flex-row-reverse chat-mb-4">
    <div class="chat-flex-none chat-flex chat-flex-col chat-items-center chat-space-y-1 chat-ml-4">
      <div
          class="chat-flex chat-items-center chat-justify-center chat-h-10 chat-w-10 chat-rounded-full chat-flex-shrink-0 chat-bg-gray-500 chat-text-gray-200">
        {{ history.name.charAt(0).toUpperCase() }}
      </div>
    </div>
    <div class="chat-flex-1 chat-bg-indigo-100 chat-text-gray-800 chat-p-2 chat-rounded-lg chat-mb-2 chat-relative">
      <div class="chat-flex chat-flex-wrap" v-if="history?.files">
        <a :href="`https://cdn.bitfiring.com/bitchat/${file.path}${file.name}.${file.extension}`" target="_blank" :download="file.original_name" v-for="file in history.files" class="chat-block chat-relative chat-p-1 chat-h-24 chat-w-full">
          <article tabindex="0"
            ...
            <section
              ...
              <div class="chat-flex">
                ...
                <p class="chat-relative chat-z-5 chat-p-1 chat-size chat-text-xs chat-text-gray-700">{{ file.size }}kb</p>
                <button
      ...
      <!-- arrow -->
      <div
          class="chat-absolute chat-right-0 chat-top-1/2 chat-transform chat-translate-x-1/2 chat-rotate-45 chat-w-2 chat-h-2 chat-bg-indigo-100"></div>
      <!-- end arrow -->
    </div>
  </div>
