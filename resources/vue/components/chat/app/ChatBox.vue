            ...
            <div class="chat_area" style="height: 350px;">
                <ul class="chat-history">
                    <component v-for="msg in history" :key="msg.id" :content="msg"
                               :is="`message-${msg.sender === 'user' ? 'me' : 'other'}`"></component>
                </ul>
                <p class="typing-status"><b v-if="typing">Typing ...</b></p>
            </div>
        </div>
        <!--chat_area-->
        <div v-if="!end_session" class="message_write">
                 <textarea @keypress.enter="onMessage" @keypress="onTyping" v-model="message" class="chat-input-box"
