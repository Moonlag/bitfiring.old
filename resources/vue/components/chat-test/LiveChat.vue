<template>
  <button-chat @onOpen="onToggle" :open="open"></button-chat>
  <div v-if="!open" class="live-chat">
    ...
    <div class="panel panel-default tricky_chat_widget modern-widget">
      ...
      <div class="panel-body">
        <ChatBox ref="scroll" v-if="connection && isOnline"
                 :history="chat_history"
                 :typing="typing"
                 :endSession="endSession"
                 @onMessage="newMessage"
        />
        <OfflineArea ref="offlineArea" v-else-if="!isOnline" @onOffline="offlineMessage"/>
        <LoginArea v-else @onLogin="onLogin"/>
      </div>
...
import OfflineArea from "./app/OfflineArea";
...
const chat_history = reactive([])
let sessionID = localStorage.getItem("sessionChat");
const typing = false
const offlineArea = ref(null)
...
io.on("session", ({userID, sessionID, username, email, online}) => {
  io.auth = {userID, sessionID, username, email};
  localStorage.setItem("sessionChat", sessionID);
  io.userID = userID;
  connection.value = true
});

io.on("chat history", (history) => {
  chat_history.push(...history)
});

io.on("private message", (message) => {
  chat_history.push(message)
  setTimeout(() => scroll.value.scrollEnd(), 100)

});

io.on("end session", (message) => {
...
function newMessage(message) {
  ...
  setTimeout(() => scroll.value.scrollEnd(), 100)
  io.emit('private message', {content: message})
}
...
async function offlineMessage(message, auth) {
  axios.post('https://bitchat.bitfiring.com:3002/offline_message', {
    ...
    offlineArea.value.showAlert()
    offlineArea.value.clearMessage()
  }).catch(e => {
    console.log(e)
  });
}
